...
#Added for testing TODO 8
RFM = features.RatioFeatureMatcher()

image = np.array(Image.open('resources/triangle1.jpg'))
image2 = np.array(Image.open('resources/triangle2.jpg')) #Added for testing Todo 7 and Todo 8

grayImage = cv2.cvtColor(image.astype(np.float32)/255.0, cv2.COLOR_BGR2GRAY)
grayImage2 = cv2.cvtColor(image2.astype(np.float32)/255.0, cv2.COLOR_BGR2GRAY) #Added for testing Todo 7 and Todo 8
def compute_and_save():
    ...
    (a1,b1) = HKD.computeHarrisValues(grayImage2) # Todo1
    c1 = HKD.computeLocalMaxima(a1) # Todo2
    d1 = HKD.detectKeypoints(image2) # Todo3
    e1 = SFD.describeFeatures(image2, d1) # Todo 4
    f1 = MFD.describeFeatures(image2, d1) # Todo 5,6

    g = SSDFM.matchFeatures(f,f1) #Todo 7
    h = RFM.matchFeatures(f,f1) # Todo 8

    d_proc = pickle_cv2(d)
    #np.savez('resources/arrays',a=a,b=b,c=c,d_proc=d_proc,e=e,f=f)
    np.savez('resources/arrays',a=a,b=b,c=c,d_proc=d_proc,e=e,f=f,g=g,h=h)
...
#try_this(7, SSDFM.matchFeatures, d, compare_cv2_points, image)

#try_this(8, RFM.matchFeatures, d, compare_cv2_points, image)

#todo, run, truth, compare, *args, **kargs